...
pn.extension(loading_spinner='dots', loading_color='#00204e',
...
def hvplot_df_line(df, x, y, title, dic_opts, color='gray', groupby=None):
    ...
    if groupby != None:
        dynamic_map = df.hvplot.line(x=x, y=y, title=title, color=color, groupby=groupby,
                                     responsive=True, min_height=400, hover_cols=[y, x, groupby])
    else:
        dynamic_map = df.hvplot.line(x=x, y=y, title=title, color=color,
                                     groupby=groupby, responsive=True, min_height=400, hover_cols='all')
...
def hvplot_df_max_min_avg_line(df, x, y, title, dic_opts):
    ...
    dynamic_map = df.hvplot.line(x=x, y=['max', 'min', 'avg'], title=title,
                                 responsive=True, min_height=400, hover_cols=['x', 'y', 'Variable'])
...
def plot_data(data, x, y, title, xlabel, ylabel, groupby, cmap_custom, clim):
    ...
    all_channels_scatter_plot = hvplot_df_scatter(data, x=x, y=y, title=title, color=y, cmap=cmap_custom,  size=20, marker='o', dic_opts={
                                                  'padding': 0.1, 'tools': [''], 'xlabel': xlabel, 'alpha': 0.25, 'ylabel': ylabel, 'clim': clim}, rasterize=True, dynamic=False)

    # Juntamos gráfico de lineas y scatters
    composite_plot = lines_plot * single_channel_scatter_plot * \
        all_channels_scatter_plot * max_line_plot
...
def create_dashboard(template):
    ...
    pacta_temp_plot_panel = create_plot_panel(pacta_temperature_data, 'PACTA Temperature', date_picker, 'scb_pixel_temperature',
                                              'avg', 'date', 'channel', 'temperature', 'Time', 'Temperature (ºC)', True, False, cmap_temps, (0, 30), template)
    scb_temp_plot_panel = create_plot_panel(scb_temperature_data, 'SCB Temperature', date_picker, 'scb_temperature',
                                            'avg', 'date', 'module', 'temperature', 'Time', 'Temperature (ºC)', False, False, cmap_temps, (0, 30), template)
    scb_humidity_plot_panel = create_plot_panel(scb_humidity_data, 'SCB Humidity', date_picker, 'scb_humidity',
                                                'avg', 'date', 'module', 'humidity', 'Time', 'Humidity (%)', False, False, cmap_humidty, (0, 80), template)
    scb_anode_current_plot_panel = create_plot_panel(scb_anode_current_data, 'Anode Current', date_picker, 'scb_pixel_an_current',
                                                     'avg', 'date', 'channel', 'anode', 'Time', 'Anode Current (µA)', False, False, cmap_anode, (0, 100), template)
...
if __name__ == '__main__':
    pn.serve(get_user_page, port=5006, show=False, static_dirs={'images': './images'}, admin=True, title='Clusco Reports',
             threaded=True, n_threads=0, check_unused_sessions_milliseconds=5000, unused_session_lifetime=5000, log_level='debug')
